...
public class CashBox implements CashContainer {
  ...
  @Override
  public int getCountFor(Currency currency, int denomination) {
    if (currency.equals(this.currency) && denomination == this.denomination)
      return banknoteCount;
    return nextCashBox == null ? 0 : nextCashBox.getCountFor(currency, denomination);
  }
   ...
   */
  @Override
  public CashContainer clone() {
    CashContainer next = null;
    if (nextCashBox != null) next = nextCashBox.clone();
    return new CashBox(currency, denomination, banknoteCount, next, logger);
  }
   ...
   */
  @Override
  public void refill(CashContainer template) {
    banknoteCount = template.getCountFor(currency, denomination);
    if (nextCashBox != null) nextCashBox.refill(template);
  }
